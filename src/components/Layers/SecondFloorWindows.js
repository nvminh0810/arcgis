...
export default function SecondFloorWindows(props) {
    ...
    for (let index = 0; index < subPoints.length - 1; index++) {
      if (!ignore.includes(index)) {
        ...
        for (let i = 0; i < points.length - 1; i++) {
          const p1 = [...points[i]];
          const p2 = [...points[i + 1]];
          drawWindow(p1, p2, true, i, direct, true, sg);
        }
      }
    }
  ...
  const drawWindow = (point1, point2, flag, i, direct, hasTwoRow, sg) => {
    let height = 0;
    const p1 = [...point1];
    const p2 = [...point2];
    if (i % 2 === 0 || sg === 'MN' || sg === 'ST') {
      height = 7;
    } else {
      if (flag) {
        height = 0.5;
  ...
  const drawWindow2 = (window, sg) => {
    ...
    [24.5, 31.3].forEach((item, index) => {
      const p1 = [...fPoint];
      const p2 = [...lPoint];
      const segment = calLineSegment(p1, p2, 15, direct);
      createPolygon({
        height: index === 0 ? 0.5 : 0.2,
